...
class CTMobile_WS_SyncModuleRecords extends CTMobile_WS_SaveRecord {

	static $SYNC_MODE_PUBLIC  = "PUBLIC";
	static $SYNC_MODE_PRIVATE = "PRIVATE";

	function isModePrivate(CTMobile_API_Request $request, $defmode="PRIVATE") {
		return (strcasecmp($request->get('mode', $defmode), self::$SYNC_MODE_PRIVATE) === 0);
	}

	function process(CTMobile_API_Request $request) {
		$current_user = $this->getActiveUser();
		$current_user_wsid = sprintf("%sx%s", CTMobile_WS_Utils::getEntityModuleWSId("Users"), $current_user->id);
		...
		$FETCH_LIMIT = CTMobile::config('API_RECORD_FETCH_LIMIT');
		...
		if (!empty($lastSyncTime) && $currentPage === 0) {
			...
			if ($hasAssignedToField) {
				if ($isPrivateMode) {
					...
					$andsmowneridequal = CTMobile_WS_Utils::querySecurityFromSuffix($module, $current_user);
				}
			}
