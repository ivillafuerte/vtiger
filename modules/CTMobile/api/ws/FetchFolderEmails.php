...
class CTMobile_WS_FetchFolderEmails extends CTMobile_WS_FetchRecord {
	...
	function process(CTMobile_API_Request $request) {
		...
		if ($foldername!='' && $maxEntriesPerPage!='' && $page_number!='') {
		   ...
		   if(($page_number*$maxEntriesPerPage) + 1 > $folderCheck->Nmsgs){
		   		$response->setResult(array("msg"=>"No messages found"));
		   }else{
			
			$response->setResult(array('pageInfo' =>$folder->pageInfo(),'email'=>$folderMails, 'module'=>'MailManager', 'msg'=>'','totalemail'=>$folderCheck->Nmsgs));
		   }
		}else{
