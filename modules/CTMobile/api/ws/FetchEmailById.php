...
class CTMobile_WS_FetchEmailById extends CTMobile_WS_FetchRecord {
	...
	function process(CTMobile_API_Request $request) {
			...
			$response->setResult(array('mailaction' => $mailaction,'fromname' => $fromname,'from' => $mail->from(), 'subject' => $mail->subject(),
					'msgno' => $mail->msgNo(), 'msguid' => $mail->uniqueid(), 'to' => $mail->to(),'cc' => $mail->cc(),'bcc' => $mail->bcc(),'attachments' => $attachments, 'linkedto'=> $linkedto, 'relatedrecords'=> $relatedrecords , 'maildate'=> $maildate ,'email_body'=>$mail->body(), 'module'=>'MailManager', 'msg'=>''));
			}else{
