...
class CTMobile_WS_FetchRecordsWithGrouping extends CTMobile_WS_FetchRecordWithGrouping {
	...
	function process(CTMobile_API_Request $request) {
		...
		// Alert id specified?
			...
			$alert = CTMobile_WS_AlertModel::modelWithId($alertid);
			if ($alert === false) {
	...
	function fetchAlertRecords($module, $alert) {		
		...
		if ($module == 'Calendar') {
			$eventsWSID = CTMobile_WS_Utils::getEntityModuleWSId('Events');
