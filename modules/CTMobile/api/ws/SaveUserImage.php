	...
	function process(CTMobile_API_Request $request) {
		...
		if (!empty($_FILES['imagename'])) {
			...
			if($upload_status){
				...
				$response->setResult(array("msg"=>"User Image Upload Successfully","ImageUrl"=>$ImageUrl));
				return $response;
			}else{
				$response = new CTMobile_API_Response();
				$response->setError('','Image Not Uploading, please try again');
				return $response; 
			} 
			      
        }else{
			$response = new CTMobile_API_Response();
			$response->setError('','Please upload User Image');
			return $response; 
		}
