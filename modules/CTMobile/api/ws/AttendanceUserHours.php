...
class CTMobile_WS_AttendanceUserHours extends CTMobile_WS_Controller {
		...
		if($adb->num_rows($query) == 0){
			throw new WebServiceException(WebServiceErrorCode::$RECORDNOTFOUND,"No Records found");
		} else {
			$total_hours = 0;
			$record = '';
			...
			$response->setResult(array('attendance_data'=>$recentEvent_data,'total_hours'=>$f_diffrent,'record'=>$record, 'module'=>'CTAttendance', 'msg'=>''));
		}
		return $response;
	}
